...
export default {
    "people-search-partner-mt": <IRequestParser>{
        getColumnsInfo: [],
        isRequestSupported(entry) { 
            return isLogiWoWi(entry)
        ...
        getCustomTabs(entry: Entry) {
            ...
            if (isLogiWoWi(entry)) {
                tabs.push(getSuggestionsTab(lokiWowiSuggestionsParser));
            }
            else {
...
const isLogiWoWi = (entry: Entry) => entry.request.url.includes("/api/v1/workingwith");
...
const lokiWowiSuggestionsParser = (response: any): ISuggestion[] | null => {
    if (!response.value) {
        return null;
    }
